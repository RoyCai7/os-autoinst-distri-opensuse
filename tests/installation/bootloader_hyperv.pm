...
sub run {
    ...
    my $root_nfs = 'N:';
    my $base_url = "https://openqa.suse.de/assets/";
        ...
        if (my $iso = get_var("ISO$n")) {
            ...
            for my $isopath ("iso", "iso\\fixed") {
                ...
                if ($mount_result->{success}) {
                    ...
                    my $iso_url = "$base_url/$isopath/$basenameiso";
                    my $powershell_cmd = "powershell -Command C:\\utils\\wget_custom.bat '$iso_url' -P '$root\\cache\\'";
        ...
        if (my $hdd = get_var("HDD$n")) {
            ...
            for my $hddpath ("hdd", "hdd\\fixed") {
                ...
                if ($mount_result->{success}) {
                    ...
                    my $hdd_url = "$base_url/$hddpath/$basenamehdd";
                    my $powershell_cmd = "powershell -Command C:\\utils\\wget_custom.bat '$hdd_url' -P '$root\\cache\\'";
                    my ($powershell_ret, $stdout, $stderr) = $svirt->run_cmd($powershell_cmd, wantarray => 1);
                }
